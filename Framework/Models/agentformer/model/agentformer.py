...
from agentformer.utils.utils import initialize_weights


def generate_ar_mask(samples, sz, agent_num, agent_mask):
    assert sz % agent_num == 0
    T = sz // agent_num
    mask = agent_mask.repeat(1, T, T)
    for t in range(T-1):
        i1 = t * agent_num
        i2 = (t+1) * agent_num
...
def generate_mask(sample_sz, tgt_sz, src_sz, agent_num, agent_mask):
    assert tgt_sz % agent_num == 0 and src_sz % agent_num == 0
    mask = agent_mask.repeat(1, tgt_sz // agent_num, src_sz // agent_num)
    return mask
...
class PositionalAgentEncoding(nn.Module):
    ...
    def forward(self, x, num_a, agent_enc_shuffle=None, t_offset=0, a_offset=0):
        ...
        if self.concat:
            feat = [x, pos_enc.unsqueeze(0).repeat(x.size(0), 1, x.size(2), 1)]
...
class ContextEncoder(nn.Module):
    ...
    def forward(self, data):
        ...
        tf_in = self.input_fc(traj_in.view(traj_in.shape[0], -1, traj_in.shape[-1])).view(traj_in.shape[0],-1, 1, self.model_dim)
        ...
        src_mask = generate_mask(tf_in.shape[0], tf_in.shape[1], tf_in.shape[1], data['agent_num'], src_agent_mask).to(tf_in.device)
        
        data['context_enc'] = self.tf_encoder(tf_in_pos, mask=src_mask, num_agent=data['agent_num'])
        
        context_rs = data['context_enc'].view(data['batch_size'], -1, data['agent_num'], self.model_dim)
        # compute per agent context
        if self.pooling == 'mean':
            data['agent_context'] = torch.mean(context_rs, dim=1)
        else:
            data['agent_context'] = torch.max(context_rs, dim=1)[0]
...
class FutureEncoder(nn.Module):
    ...
    def forward(self, data, reparam=True):
        ...
        mem_mask = generate_mask(tf_in.shape[0], tf_in.shape[1], data['context_enc'].shape[1], data['agent_num'], mem_agent_mask).to(tf_in.device)
        tgt_mask = generate_mask(tf_in.shape[0], tf_in.shape[1], tf_in.shape[1], data['agent_num'], tgt_agent_mask).to(tf_in.device)
        
        tf_out, _ = self.tf_decoder(tf_in_pos, data['context_enc'], memory_mask=mem_mask, tgt_mask=tgt_mask, num_agent=data['agent_num'])
        tf_out = tf_out.view(traj_in.shape[0], traj_in.shape[1], -1, self.model_dim)
...
class FutureDecoder(nn.Module):
    ...
    def decode_traj_ar(self, data, mode, context, pre_motion, pre_vel, pre_motion_scene_norm, z, sample_num, need_weights=False):
        ...
        dec_in = dec_in.view(data['batch_size'], -1, sample_num, dec_in.shape[-1])
        z_in = z.view(data['batch_size'], -1, sample_num, z.shape[-1])
        in_arr = [dec_in, z_in]
        for key in self.input_type:
            if key == 'heading':
        ...
        for i in range(self.future_frames):
            tf_in = self.input_fc(dec_in_z.view(dec_in_z.shape[0], -1, dec_in_z.shape[-1])).view(dec_in_z.shape[0], dec_in_z.shape[1], -1, self.model_dim)
            agent_enc_shuffle = data['agent_enc_shuffle'] if self.agent_enc_shuffle else None
            ...
            # tf_in_pos = tf_in
            mem_mask = generate_mask(tf_in.shape[0], tf_in.shape[1], context.shape[1], data['agent_num'], mem_agent_mask).to(tf_in.device)
            tgt_mask = generate_ar_mask(tf_in_pos.shape[0], tf_in_pos.shape[1], agent_num, tgt_agent_mask).to(tf_in.device)
            ...
            out_tmp = tf_out.view(tf_out.shape[0], -1, tf_out.shape[-1])
            if self.out_mlp_dim is not None:
                out_tmp = self.out_mlp(out_tmp)
            seq_out = self.out_fc(out_tmp).view(tf_out.shape[0], tf_out.shape[1], -1, self.forecast_dim)
            if self.pred_type == 'scene_norm' and self.sn_out_type in {'vel', 'norm'}:
                norm_motion = seq_out.view(seq_out.shape[0], -1, agent_num * sample_num, seq_out.shape[-1])
                if self.sn_out_type == 'vel':
                    norm_motion = torch.cumsum(norm_motion, dim=1)
                ...
                seq_out = seq_out.view(tf_out.shape[0], tf_out.shape[1], -1, seq_out.shape[-1])
            if self.ar_detach:
        ...
        seq_out = seq_out.view(seq_out.shape[0], -1, agent_num * sample_num, seq_out.shape[-1])
        data[f'{mode}_seq_out'] = seq_out

        if self.pred_type == 'vel':
            dec_motion = torch.cumsum(seq_out, dim=1)
        ...
        dec_motion = dec_motion.transpose(1,2).contiguous()       # M x frames x dims
        if mode == 'infer':
            dec_motion = dec_motion.view(dec_motion.shape[0], -1, sample_num, *dec_motion.shape[2:])        # M x Samples x frames x 3
    ...
    def forward(self, data, mode, sample_num=1, autoregress=True, z=None, need_weights=False):
        context = data['context_enc'].repeat_interleave(sample_num, dim=2)       # 80 x 64
        pre_motion = data['pre_motion'].repeat_interleave(sample_num, dim=2)             # 10 x 80 x 2
        pre_vel = data['pre_vel'].repeat_interleave(sample_num, dim=2) if self.pred_type == 'vel' else None
        pre_motion_scene_norm = data['pre_motion_scene_norm'].repeat_interleave(sample_num, dim=2)
        
        # p(z)
        prior_key = 'p_z_dist' + ('_infer' if mode == 'infer' else '')
        if self.learn_prior:
            h = data['agent_context'].repeat_interleave(sample_num, dim=1)
            ...
            if self.z_type == 'gaussian':
                data[prior_key] = Normal(mu=torch.zeros(pre_motion.shape[0], pre_motion.shape[2], self.nz).to(pre_motion.device), 
                                         logvar=torch.zeros(pre_motion.shape[0], pre_motion.shape[2], self.nz).to(pre_motion.device))
            else:
                data[prior_key] = Categorical(logits=torch.zeros(pre_motion.shape[0], pre_motion.shape[2], self.nz).to(pre_motion.device))
...
class AgentFormer(nn.Module):
    def __init__(self, cfg):
        ...
        self.max_train_agent = cfg.get('max_train_agent', 1000)
        ...
        # map encoder
        if self.use_map:
            print("Map encoding not supproted here")
    ...
    def set_data(self, data):
        device = self.device
        if self.training and len(data[0]['pre_motion_3D']) > self.max_train_agent:
            raise AttributeError("To many agents in scene for model to handle.")
        elif self.training and len(data[0]['pre_motion_3D'])*len(data) > self.max_train_agent:
            in_data = []
            ind = np.random.choice(len(data), int(np.floor(self.max_train_agent/len(data[0]['pre_motion_3D'])))).tolist()
            for i in ind:
        ...
        self.data['batch_size'] = len(in_data)
        self.data['agent_num'] = len(in_data[0]['pre_motion_3D'])
        # num_samples * num_timesteps * num_agents * num_dim
        self.data['pre_motion'] = torch.stack([torch.stack(in_data[i]['pre_motion_3D'], dim=0).transpose(0, 1).contiguous()
                                               for i in range(self.data['batch_size'])]).to(device)
        self.data['fut_motion'] = torch.stack([torch.stack(in_data[i]['fut_motion_3D'], dim=0).transpose(0, 1).contiguous()
                                               for i in range(self.data['batch_size'])]).to(device)
        self.data['fut_motion_orig'] = torch.stack([torch.stack(in_data[i]['fut_motion_3D'], dim=0)
                                                    for i in range(self.data['batch_size'])]).to(device) # future motion without transpose
        
        self.data['fut_mask'] = torch.stack([torch.stack(in_data[i]['fut_motion_mask'], dim=0)
                                             for i in range(self.data['batch_size'])]).to(device) 
        self.data['pre_mask'] = torch.stack([torch.stack(in_data[i]['pre_motion_mask'], dim=0)
                                             for i in range(self.data['batch_size'])]).to(device) 
        
        self.data['scene_orig'] = torch.tensor([0.0,0.0]).to(device)
        ...
        cur_motion_shape = self.data['cur_motion'][:,0].shape
        mask = torch.zeros([cur_motion_shape[0], cur_motion_shape[1], cur_motion_shape[1]]).to(device)
